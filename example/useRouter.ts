...
mergeRouter
  .get(
    "",
    async ctx => (ctx.body = `${ctx.router.name} in ${ctx.router.route}`)
  )
  .post(
    "",
    async ctx => (ctx.body = `POST ${ctx.router.name} in ${ctx.router.route}`)
  )
  .get(
    "me/:name",
    async ctx =>
      (ctx.body = `${ctx.router.name} dis ${ctx.router.params.name} in ${
        ctx.router.route
      }`)
  );
...
router
  .get("/:id", async ctx => {
    ctx.body = `we have ${JSON.stringify(ctx.router.params)} in ${
      ctx.router.route
    }`;
  })
